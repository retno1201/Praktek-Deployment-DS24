...
def main():
    background = """<div style ='background-color:black; padding:13px'>
                    <h1 style ='color:white'>Model Deployment Loan Eligibility Prediction</h1>
                </div>"""
    ...
    if button:
        ...
        result = predict(gender,married,dependent,education,self_employed,applicant_income,Coapplicant_income,
                         loan_amount,loan_amount_term,credit_history,propertyArea)
...
def predict(gender,married,dependent,education,self_employed,applicant_income,Coapplicant_income,
    ...
    #Membuat prediksi
    prediction = RF_Model.predict([[gen, mar, dep, edu, sem, applicant_income, cap, lam, loan_amount_term, credit_history, pro]])
